...
@section('content')
    ...
    <div class="container">
        <div class="card border-success">
            <div class="card-header font-weight-bold bg-gradient-primary"><h3 class="mb-0">Update Port Usage</h3></div>
            <div class="card-body">
                @if($errors->any())
                ...
                <form action="{{route('ports.update', isset($ports)?$ports->id:'')}}" method="post">
                    @csrf
                    @method('put')
                    <div class="form-group">
                        <label for="id">Id</label>
                        <input type="number" class="form-control" id="id"
                               name="id" value="{{$ports->id}}">
                    </div>
                    <div class="form-group">
                        <label for="name">Name</label>
                        <input type="text" class="form-control" id="name"
                               name="name" value="{{$ports->name}}">
                    </div>
                    <div class="form-group">
